...
async function paginate<
  ...
  V
>(endpoint: (params: P) => Promise<V[]>, params: P, count?: number) {
  const items: V[] = [];

  params.page = 0;
  params.per_page = 100;
  while (items.length == params.page * params.per_page || (count != null && items.length <= count)) {
    params.page++;
    items.push(...(await endpoint(params)));
  }
